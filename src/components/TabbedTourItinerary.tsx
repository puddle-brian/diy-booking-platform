                              ...
                              <table className="w-full min-w-[1000px] table-fixed">
                                ...
                                <tbody className={expandedDividerClass}>
                                    ...
                                    {sameDateSiblings.map((siblingEntry, siblingIndex) => {
                                      ...
                                      let siblingRequestBids: VenueBid[] = [];
                                      ...
                                      if (siblingRequest.isVenueInitiated && siblingRequest.originalOfferId) {
                                        ...
                                        if (originalOffer) {
                                          ...
                                          siblingRequestBids = [syntheticBid];
                                        }
                                        ...
                                        siblingRequestBids = allBidsOnRequest;
                                      } else {
                                        // For regular artist-initiated requests, use normal bid filtering
                                        siblingRequestBids = venueBids.filter(bid => bid.showRequestId === siblingRequest.id && !declinedBids.has(bid.id));
                                      }

                                      // Determine status for sibling entry (same logic as parent)
                                      const siblingHasAcceptedBid = siblingRequestBids.some((bid: VenueBid) => 
                                        bid.status === 'accepted' || (bid as any).holdState === 'ACCEPTED_HELD'
                                      );
                                      const siblingHasActiveHold = siblingRequestBids.some((bid: VenueBid) => 
                                        (bid as any).holdState === 'HELD' || (bid as any).holdState === 'FROZEN'
                                      );
                                      const isSiblingHeldBidRequest = (siblingRequest as any).isHeldBid;
                                      
                                      let siblingRequestStatus: 'confirmed' | 'pending' | 'hold' | 'accepted' = 'pending';
                                      if (siblingHasAcceptedBid) {
                                        siblingRequestStatus = 'accepted';
                                      } else if (siblingHasActiveHold || isSiblingHeldBidRequest) {
                                        siblingRequestStatus = 'hold';
                                      }
                                      ...
                                      return (
                                        <tr key={`sibling-${siblingRequest.id}-${siblingIndex}`} className="bg-gray-50 hover:bg-gray-100">
                                          ...
                                          <td className="px-4 py-1 w-[19%]">
                                            <div className="text-sm font-medium text-gray-900 truncate">
                                              {artistId ? (
                                                ...
                                                siblingRequest.isVenueInitiated ? (
                                                  // For venue-initiated offers, show the venue name as clickable link
                                                  (siblingRequest as any).venueId && (siblingRequest as any).venueId !== 'external-venue' ? (
                                                    <a 
                                                      href={`/venues/${(siblingRequest as any).venueId}`}
                                                      className="text-blue-600 hover:text-blue-800 hover:underline"
                                                      title="View venue page"
                                                      onClick={(e) => e.stopPropagation()}
                                                    >
                                                      {(siblingRequest as any).venueName}
                                                    </a>
                                                  ) : (
                                                  ...
                                                  // For artist-initiated requests, show bid count or venue-specific info
                                                  siblingRequestBids.length === 1 && siblingRequestBids[0].venueId !== 'external-venue' ? (
                                                    <a 
                                                      href={`/venues/${siblingRequestBids[0].venueId}`}
                                                      className="text-blue-600 hover:text-blue-800 hover:underline"
                                                      title="View venue page"
                                                      onClick={(e) => e.stopPropagation()}
                                                    >
                                                      {siblingRequestBids[0].venueName}
                                                    </a>
                                                  ) : (
                                                    <span className="text-gray-600 text-sm">
                                                      {siblingRequestBids.length === 0 ? 'No bids yet' : 
                                                       siblingRequestBids.length === 1 ? siblingRequestBids[0].venueName :
                                                       `${siblingRequestBids.length} competing venues`}
                                                    </span>
                                                  )
                                                )
                                              ) : (
                                                // Show artist info for venue pages
                                                siblingRequest.artistId && siblingRequest.artistId !== 'external-artist' ? (
                                                  <a 
                                                  ...
                                                  <span>{siblingRequest.artistName}</span>
                                                )
                                              )}
                                            </div>
                                          </td>
                                          <td className="px-4 py-1 w-[10%]">
                                            {(() => {
                                              // If viewing as a venue, show venue's specific bid status
                                              if (venueId && siblingRequestBids.length > 0) {
                                                const venueBid = siblingRequestBids.find(bid => bid.venueId === venueId);
                                                if (venueBid) {
                                                  const statusBadge = getBidStatusBadge(venueBid);
                                                  return (
                                                    <span className={statusBadge.className}>
                                              ...
                                              // Use the same status logic as parent entries
                                              if (siblingHasAcceptedBid) {
                                                return (
                                                  <span className="inline-flex px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-800">
                                                    Accepted
                                              ...
                                              if (siblingHasActiveHold || isSiblingHeldBidRequest) {
                                                return (
                                                  <span className="inline-flex px-2 py-0.5 text-xs font-medium rounded-full bg-violet-100 text-violet-700">
                                                    Hold
                                                  </span>
                                          ...
                                          <td className="px-4 py-1 w-[8%]">
                                            <div className="flex items-center space-x-1">
                                              {/* Document button for sibling requests */}
                                            ...
                                            <div className="flex items-center space-x-2">
                                              ...
                                              {venueId && (
                                                <DeleteActionButton
                                                  ...
                                                  venueOffers={venueOffers as any}
                                                  venueBids={venueBids}
                                                  isLoading={state.deleteLoading === siblingRequest.id}
                                                  onDeleteRequest={handleDeleteShowRequest}
                                                  onOfferAction={(offer, action) => handleOfferAction(offer as any, action)}
                                                  onBidAction={(bid, action, reason) => handleBidAction(bid as any, action, reason)}
                                                />
